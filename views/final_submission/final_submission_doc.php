...
<p>
    Throughout development, there were small details of the project that were not completed by their specified deadlines. The group had to reorganize and shift priority of tasks. Some problems encountered during development included:
...
<h2>Successes During Development</h2>
<p>
...
<p>
    The required functionality exists for both versions. However, due to time constraints, we were unfortunately not able to fully implement exception handling (or logging, in PHP) and hard-coded ids and strings are used in some places instead of using a config file (a small barrier to other institutions using the software). There are also differences between the two versions resulting from different people working on them (and fixing problems in the design separately) that would've cost too much effort to reconcile. Large portions of code are also poorly commented. Finally, due to the design of our project putting large portions of logic into static functions, we were unable to properly implement unit testing in Python, and no unit tests were written for PHP.
</p>
<h2>Conclusion</h2>
<p>
